...
export function reportGlobalWarnings(warnings) {
    if (warnings.length > 0) {
        consolePrinter.font
            .error(`✖ ${warnings.length} global ${pluralize("warning", warnings.length)}`)
            .bold()
            .print();
    }
    else {
...
function printDependencyWarnings(warnings, warningsMode) {
...
export function reportDependencyWarnings(warnings, warningsMode) {
    ...
    if (numberOfDependencyWarnings > 0) {
        ...
        printWarnOrError(warningsMode)(`✖ ${numberOfDependencyWarnings} dependency ${pluralize("warning", numberOfDependencyWarnings)}`)
            .bold()
            .print();
    }
    else {
