...
import { pipeline, convertBooleanAsCheckResult, FAILING_CHECK } from "../pipeline/index.js";
import { extractScannerPayload } from "./extract.js";
function checkGlobalWarnings(warnings) {
    return {
...
function checkDependenciesWarnings(warnings, runtimeConfiguration) {
    ...
    if (runtimeConfiguration.warnings === "error" ||
        ...
        const allDependencyWarnings = warnings.filter((dependency) => dependency.warnings.length > 0);
        return {
            result: runtimeConfiguration.warnings === "error"
    ...
    const causingErrorWarnings = new Set(Object.keys(runtimeConfiguration.warnings));
    const errorOnlyWarnings = warnings
        .map((dependency) => {
        const errorWarnings = dependency.warnings.filter((dependencyWarning) => causingErrorWarnings.has(dependencyWarning.kind));
        return {
            ...dependency,
...
function convertSeverityAsNumber(severity) {
    const severities = {
        critical: 4,
        high: 3,
        medium: 2,
...
function findAllVulnsWithEqualOrHigherSeverity(vulnerabilities, severity) {
    return vulnerabilities.filter((vuln) => convertSeverityAsNumber(vuln.severity) >=
        convertSeverityAsNumber(severity));
}
function checkDependenciesVulns(vulnerabilities, runtimeConfiguration) {
    const { vulnerabilitySeverity } = runtimeConfiguration;
    const vulnsClassifiedBySeverity = findAllVulnsWithEqualOrHigherSeverity(vulnerabilities, vulnerabilitySeverity);
    return {
